...
工具注册表包括资产与分组管理、SSH/文件操作、批量执行、SQL 与受支持的数据服务操作、Kubernetes 与 Kafka 操作、权限请求以及本地工作区工具。已安装扩展通过统一的 `ext_exec` 分发器暴露，而不是为每个扩展硬编码一个 AI 工具。

工具可用性取决于具体能力，并非每种资产都有 AI 操作辅助器。特别是，内置的 RDP 与 VNC 会话目前是交互式应用界面，不是 AI 辅助器。注册表会随应用演进，因此对话中显示的工具卡片才是当前运行版本的权威清单。
