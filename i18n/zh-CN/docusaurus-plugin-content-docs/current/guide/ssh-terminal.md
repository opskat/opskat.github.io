...
OpsKat 提供功能完善的 SSH 终端，支持分屏、SFTP 文件浏览、连接后自动执行命令、跳板机链、端口转发和代理链。
...
SSH 资产可以在交互式 shell 就绪后自动执行命令。在资产的 SSH 高级配置中填写**连接后执行命令**，多条命令用换行分隔。

- 每次连接（含重连）以及会话中的每个分屏窗格都会执行。
- 命令在 shell 稳定接管之后才派发，因此由 shell 执行，不会被行规程重复回显。
- 同时开启**重连恢复上次目录**时，先恢复目录，再执行这些命令。
- 命令写入失败只记录警告，不会因此拆掉已经建立好的终端连接。
...
为 SSH 连接配置代理链。当 SSH 服务器只能经由一到多个中间跳转访问时非常有用。

在资产的 SSH 配置中设置代理链，每一层包含：

- **层类型** — SSH 跳板机、SOCKS5 或 HTTP 隧道（各层可混用、可调序）
- **主机 / 端口** — 代理服务器地址
- **用户名 / 密码** — 可选的代理认证
