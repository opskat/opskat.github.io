...
[策略规则](/docs/guide/policy)约束的是 **AI 智能体**和 **`opsctl`** 发起的操作：被策略拒绝的操作不会执行，需要确认的会先弹窗。目前 SQL、Redis、MongoDB、etcd、Kafka、Kubernetes 和对象存储都有各自的策略类型。

本页这些桌面端控制台走的是另一条路径 —— 它们由你本人操作，而非智能体代劳，因此**不做**策略检查。真正起作用的是：

- **确认弹窗** —— SQL 编辑器在执行 `DELETE`、`DROP`、`TRUNCATE`、`ALTER` 前会要求确认；其他控制台对各自的破坏性操作同样会确认
- **只读资产** —— 标记为只读的数据库资产会以只读连接打开，写操作在驱动层就会失败

如果你希望一组语句受到放行/拦截的强制约束，请通过 AI 智能体或 `opsctl` 执行，而不是用控制台。
