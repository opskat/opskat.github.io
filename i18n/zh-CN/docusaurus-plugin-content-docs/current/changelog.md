...
- 🐛 退出确认只拦运行中任务（AI 生成、opsctl 任务），空闲的 SSH / RDP / VNC 会话不再阻挡退出；同时修复确认中断退出时 AI 会话不落盘、以及前端无响应时窗口关不掉的问题 (by @CodFrm)

**完整更新记录**: [v1.13.3...v1.13.4](https://github.com/opskat/opskat/compare/v1.13.3...v1.13.4)
...
- **下载完整性校验** — 对下载的文件做 SHA256 校验，校验失败时提示手动下载。
- **下载镜像支持** — 设置中新增镜像源选择，支持自动回退与跳过校验。
- **AI 思考过程展示** — AI 对话新增思考过程展示、消息队列、停止/重试与错误恢复。
- **终端右键菜单与搜索** — 终端支持右键菜单和搜索；MySQL 支持 TLS，密码支持回显。
- **边缘展开条** — 用于侧边栏导航的边缘热区组件。
- **系统状态面板** — 新增启动状态注册表，设置页增加状态区块，迁移失败时弹出提示。

### Bug 修复

- 修复 DeepSeek 等 OpenAI 兼容 API 返回 400 的问题。
- 修复 CI 中 golangci-lint v2 的格式化与 staticcheck 问题。
- 移除多余的 `nolint gosec` 指令。
- 修复 UpdateSection 的 prettier 格式问题。

### CI/CD

- 发布时生成 `release-info.json`，供镜像源检查更新。

### 其他

- 重写 README，突出使用场景与安全审计。

**完整更新记录**: [v1.1.0...v1.1.1](https://github.com/opskat/opskat/compare/v1.1.0...v1.1.1)
